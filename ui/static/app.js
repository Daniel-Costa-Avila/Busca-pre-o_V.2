(function () {
    ...
    const timelineCompact = document.getElementById("timeline-compact");
    ...
    const btnOpenRunPane = document.getElementById("btn-open-run-pane");
    const btnOpenStatusPane = document.getElementById("btn-open-status-pane");
    ...
    function createTimelineItem(message, kind) {
        const li = document.createElement("li");
        li.classList.add("timeline-item");
        if (kind === "error") li.classList.add("timeline-item--error");
        if (kind === "success") li.classList.add("timeline-item--success");
    ...
    function pushTimelineItem(listEl, li, limit) {
        if (!listEl) return;
        listEl.prepend(li);
    ...
    function logEvent(message, kind) {
        const liFull = createTimelineItem(message, kind);
        pushTimelineItem(timeline, liFull, 18);

        // Visao executiva: lista compacta para leitura rapida no Resumo.
        const liCompact = createTimelineItem(message, kind);
        pushTimelineItem(timelineCompact, liCompact, 8);
    }
    ...
    function setDailyFeedback(text, kind) {
    ...
    // Chip "Tempo": mostra tempo de execucao quando RUNNING, senao mostra o relogio local.
    ...
    function updateTimeChip() {
        if (!chipTime) return;
        if (state.jobStatus === "RUNNING" && state.jobCreatedAtSec) {
            const nowSec = Math.floor(Date.now() / 1000);
            chipTime.textContent = `Tempo: ${fmtDuration(nowSec - state.jobCreatedAtSec)}`;
            return;
        }
        chipTime.textContent = `Hora: ${new Date().toLocaleTimeString("pt-BR")}`;
    }
    ...
    function updateJobUI(statusData) {
        ...
        state.jobStatus = String(statusData.status || "").trim();
        state.jobCreatedAtSec = Number(statusData.created_at || 0) || 0;
        updateTimeChip();
    ...
    async function addDailyTime() {
        ...
        if (!/^\d{1,2}:\d{2}$/.test(value)) {
            ...
            setDailyFeedback("Horario invalido. Use o formato HH:MM (ex.: 09:30).", "error");
            return;
        }

        if (btnDailyAddTime) {
            btnDailyAddTime.disabled = true;
        }
        setDailyFeedback("Adicionando horario...", "info");
        try {
            ...
            setDailyFeedback(`Horario ${value} adicionado.`, "success");
            await refreshDaily();
        } catch (error) {
            logEvent(`Falha ao adicionar horario: ${error.message}`, "error");
            setDailyFeedback(`Falha ao adicionar horario: ${error.message}`, "error");
        } finally {
            if (btnDailyAddTime) {
                btnDailyAddTime.disabled = false;
            }
        }
    }
    ...
    async function startRun(event) {
        ...
        try {
            ...
            const data = await response.json();
            if (data.mode === "tampermonkey_queue" && data.task_id) {
                setFeedback(`Tarefa enviada para Tampermonkey: ${data.task_id} (${data.entries_count || 0} entradas).`, "success");
                logEvent(`Tarefa ${data.task_id} criada para fila Tampermonkey.`, "success");
                return;
            }
            ...
            if (data.ml_auto_routed) {
                const taskId = data.ml_task_id || "";
                const count = Number(data.ml_entries_count || 0);
                logEvent(`Mercado Livre roteado automaticamente para fila Tampermonkey: ${taskId} (${count} entradas).`, "info");
            }
            await refreshStatus();
        } catch (error) {
    ...
    function bindEvents() {
        if (btnOpenRunPane) {
            btnOpenRunPane.addEventListener("click", function () {
                activateFeaturePane("feature-pane-run");
                if (fileInput) {
                    fileInput.focus();
                }
            });
        }
        if (btnOpenStatusPane) {
            btnOpenStatusPane.addEventListener("click", function () {
                activateFeaturePane("feature-pane-status");
            });
        }
        if (runForm) {
        ...
        if (dailyNewTime) {
            dailyNewTime.addEventListener("keydown", function (event) {
                if (event.key === "Enter") {
                    event.preventDefault();
                    addDailyTime();
                }
    ...
    function init() {
        ...
        syncAdminSessionUi();
        bindEvents();
        if (btnDownloadInputExample) {
            btnDownloadInputExample.href = withToken("/download/input-example");
        }
    ...
    try {
        ...
        if (dailyFeedback) {
            setDailyFeedback(`Falha ao inicializar o painel: ${message}`, "error");
        }
        // Ajuda no diagnostico quando a UI esta embutida em iframe (ex.: Streamlit).
