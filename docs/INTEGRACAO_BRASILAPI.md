...
- Câmbio: lista de moedas disponíveis.
- Taxas: Selic, CDI, IPCA e outras taxas disponíveis, por lista ou sigla.
...
- Bancos v1: consulta por código ou listagem.
- Feriados v1: feriados nacionais por ano.
- IBGE municípios v1: municípios por UF.

O menu **Central de Dados Públicos** usa `integrations/brasilapi_panel.py` e `integrations/brasilapi.py`, com as consultas agrupadas em abas por assunto (Endereço e Localização, Empresas e Pessoas, Fiscal e Comércio, Financeiro, Institucional). As consultas saem do servidor Streamlit para `https://brasilapi.com.br/api`, sem enviar o token interno do Busca Preço. Não é necessário executar o projeto Node da pasta `Api Brasil` para consumir o serviço público. Referência: https://brasilapi.com.br/docs e documentação local em `Api Brasil/pages/docs/doc`.
